...
void test_time()
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        matmul_plain(matrix1, matrix2, matrix3);
        // cblas_sgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, SIZE_ROW, SIZE_ROW, SIZE_COL, 1, matrix1->data, SIZE_COL, matrix2->data, SIZE_ROW, 0, matrix3->data, SIZE_ROW);
        end = omp_get_wtime();
